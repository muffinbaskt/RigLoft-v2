...
// Merging duplicate items (usually after Receiving created a new one that
// turned out to already exist under a slightly different name), and the
// receipt-batch factory used by both the standalone Receiving screen and
// combining separately-scanned pages of the same document.
...
export function mergeJobItems(items, sourceId, targetId) {
...
export function mergeLoveListItems(items, sourceId, targetId) {
  const sourceIdx = items.findIndex((i) => i.id === sourceId);
  const targetIdx = items.findIndex((i) => i.id === targetId);
  if (sourceIdx === -1 || targetIdx === -1 || sourceId === targetId) return items;
...
export function newReceiptBatch(photoUrl, path, lines, meta = {}) {
  return {
    id: uniqueId(),
